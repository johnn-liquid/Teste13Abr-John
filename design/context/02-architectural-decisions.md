# 4.4 - Design System: Architectural Decisions (Novo)
...
LuriDS is dark mode only. Light mode tokens exist as reference but are never the active theme. This is an architectural decision, not an aesthetic one.
...
Any generated screen uses dark mode tokens exclusively. There is no light mode fallback.
...
- 4.3 Interaction Model — Human-in-the-Loop patterns that require handoff motion
- 4.5 Experience Context — trust states that determine motion intensity
- 4.6 Microcopy & Tone of Voice — copy decisions that follow the same rationale pattern
