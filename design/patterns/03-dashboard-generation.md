---
layer: ux-dashboard-context
question: How should agents compose and generate dashboards in Liquid OS?
updated: 2026-04-09
---
...
| **Pipeline Dashboard** | What is the current status and progression of this process pipeline? | Coordinator | Stage-based list (kanban-like) + KPI (per stage) + Evolution chart | Individual entity credit data |
...
### 3.2 Compositional Judgment

The number and type of elements in a dashboard are not fixed by rule — they are chosen strategically by the agent based on the complexity of the answer the user needs.

A dashboard answering a simple operational question may need only 1 KPI and 1 supporting text. A dashboard synthesizing a complex multi-dimensional analysis may need multiple KPIs, charts of different types, a datatable, and a closing document. Both are correct if each element is analytically justified.

**The agent must be able to answer "why is this element here?" for every artifact included.** If the answer is "because it looks complete" or "by default," the element should be removed.

**Structural rules that always apply regardless of complexity:**

- Two charts of the same structural type (e.g., bar + bar, line + line) are never permitted in the same dashboard — they create visual redundancy without adding analytical distinction. If two charts are needed, they must be of structurally different types (e.g., bar + line, area + donut).
- If the dashboard contains 3 or more visual artifacts, it must close with an `ArtifactDocument` that interprets the combined picture and states a suggested action.
- A minimum viable dashboard requires at least 1 anchor KPI and 1 element that explains the context behind it.
...
8. VERIFY_DATA
   → All financial figures must follow the locale configured for the Workspace
     (currency symbol, decimal separator, thousands separator).
     Default fallback if locale is not set: BRL (R$ 1.234.567,89).
